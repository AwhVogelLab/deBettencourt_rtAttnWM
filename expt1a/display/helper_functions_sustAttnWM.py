...
def get_specs(subj_name='mmddyyn_wmLoadMem02'):
    ...
    dictDlg = gui.DlgFromDict({'Participant number': 0,     #subject name: month, day, year, number, and project name
                                'Environment':['other','booth','imac','edslab'],    #which environment (aka monitor name in Tools > Monitor Center)
    ...
    if gui_specs['env']=='imac':
        gui_specs['project_dir']='/Users/megan/Documents/projects/sustAttnWM01/'
    elif gui_specs['env']=='booth':
        gui_specs['project_dir']='C:/Users/AwhVogelLab/Desktop/Megan/sustAttnWM01/'
    elif gui_specs['env']=='edslab':
        gui_specs['project_dir']='/Users/edslab/Documents/projects/sustAttnWM01/'
    elif gui_specs['env']=='other':
        gui_specs['project_dir']='./../'
        print 'unknown environment selected in the GUI'
...
def write_to_csv_data_file_cd(data_file,iblock,itrial,dsgn,headerLine=False):
    if headerLine:
        ...
        data_file.write(str(iblock) + ','+ str(itrial) + ',' + datetime.utcnow().strftime("%H:%M:%S:%f") + ',' + str(dsgn.cd_setsize) +',' + 
                        str(dsgn.cd_same_probe[itrial]) + ',' + str(dsgn.cd_encarray_colorind[itrial]) + ',' + str(dsgn.cd_encarray_quad[itrial]) + ',' +
                        str(dsgn.cd_encarray_mindist[itrial]) + ',' + str(dsgn.cd_memprobe_ind[itrial]) + ',' + str(dsgn.cd_memprobe_origcolorind[itrial]) + ',' +
                        str(dsgn.cd_memprobe_probecolorind[itrial]) + ',' + str(dsgn.cd_memprobe_probecolorrgb[itrial]) + ',' + 
                        str(dsgn.cd_memprobe_quad[itrial]) + ',' + str(dsgn.cd_memprobe_x[itrial]) + ',' + str(dsgn.cd_memprobe_y[itrial]) + ',' +
                        str(dsgn.cd_correct_response[itrial]) + ',' + str(dsgn.cd_actual_response[itrial]) + ',' + dsgn.cd_actual_response_string[itrial] + ',' +
                        str(dsgn.cd_acc[itrial]) + ',' + str(dsgn.cd_rt[itrial]) + ',' + 
                        str(dsgn.cd_time_blockstart) + ',' + str(dsgn.cd_time_encarray_onset[itrial]) + ',' +str(dsgn.cd_time_encarray_offset[itrial]) + ',' + 
                        str(dsgn.cd_time_memprobe_onset[itrial]) + ',' + str(dsgn.cd_time_memprobe_offset[itrial]))
        data_file.flush()
        data_file.write('\n')
