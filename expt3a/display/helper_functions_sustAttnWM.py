...
def get_specs(subj_name='mmddyyn_wmLoadMem02'):
    ...
    dictDlg = gui.DlgFromDict({'Participant number': '09dd18_sustAttnPrecis01',     #subject name: month, day, year, number, and project name
                                'Environment':['Dirk VU','imac','booth','edslab'],    #which environment (aka monitor name in Tools > Monitor Center)
...
def write_to_csv_data_file(data_file,iblock,itrial,dsgn,headerLine=False):
    if headerLine:
        data_file.write('block,trial,time,setsize,freqtrials,'+
                        'colorind,colordeg,colorrgb,correctresp,actualresp,acc,rts,'+
                        'probe_trials,fast_rt_trigger,slow_rt_trigger,'+
                        'rt_trailingavg,rt_runningavg,rt_runningstd,rt_runningslowthresh,rt_runningfastthresh,'+
                        'wm_mousepos,wm_respdeg,wm_respind,'+
                        'wm_colorind,wm_colorrgb,wm_colorhsv,wm_degdiff,wm_rts,'+
                        'timeBlockStart,timeEncArrayOnset,timeResponse,' + 
                        'timeProbeOnset,timeProbeOffset\n')
    else:
        data_file.write(str(iblock) +','+ str(itrial) +','+ 
                        ...
                        str(float(dsgn.trials_colorind[iblock,itrial][0])/dsgn.n*360) +','+ 
                        str(dsgn.trials_colorrgb[iblock,itrial][0]) +','+
                        ...
                        str(dsgn.wm_mousepos[iblock,itrial]) +','+ 
                        str(dsgn.wm_respdeg[iblock,itrial]) +','+ 
                        str(np.round((dsgn.wm_respdeg[iblock,itrial]/360.0)*512)) +','+   
                        str(dsgn.wm_resptexind[iblock,itrial]) +','+ 
                        str(dsgn.wm_resptexrgb[iblock,itrial]) +','+ 
                        str(dsgn.wm_resptexhsv[iblock,itrial]) +','+ 
