...
class ExperimentalDesign(object):
    ...
    def __init__(self,gui_specs):
        ...
        self.probe_trials = np.zeros((self.nblocks,self.ntrials_perblock))
        for ib in range(self.nblocks):
            self.prop_infreq_probe = .5
            self.n_probes_perblock = int(np.round(self.ntrials_perblock*(1-self.prop_freq)*self.prop_infreq_probe))
            self.n_infreq_perblock = int(np.sum(self.freq_trials[ib]==0))
            probe_trials = np.append(np.zeros(self.n_infreq_perblock-self.n_probes_perblock),
                                    np.ones(self.n_probes_perblock))
            np.random.shuffle(probe_trials)
            self.probe_trials[ib][self.freq_trials[ib]==0] = probe_trials
